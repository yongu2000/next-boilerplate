...
export default function UserLikes({ params }: { params: Promise<{ username: string }> }) {
  ...
  const fetchPosts = async (cursor?: number) => {
    ...
    try {
      ...
      const response = await postService.getUserLikes(resolvedParams.username, searchParamsWithFilters);
